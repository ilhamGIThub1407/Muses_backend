                    ...
                    <tbody>
                        {
                            news.length > 0 && news.slice((page - 1) * parPage, page * parPage).map((n, i) => <tr key={i} className='bg-white border-b' >
                                ...
                                <td className='px-6 py-4'>
                                    <img className='w-10 h-10' src={n.image} alt="" />
                                </td>
                                ...
                                <td className='px-6 py-4'>{n.date}</td>
                                {
                                    store?.userInfo?.role === 'admin' ? <td className='px-6 py-4'>
                                        {
                                            n.status === 'pending' && <span onClick={() => update_status('active', n._id)} className='px-2 py-0.5 bg-blue-100 text-blue-800 rounded-lg text-xs cursor-pointer' >{res.loader && res.id === n._id ? 'Loading...' : n.status}</span>
                                        }
                                        {
                                            n.status === 'active' && <span onClick={() => update_status('deactive', n._id)} className='px-2 py-0.5 bg-green-100 text-green-800 rounded-lg text-xs cursor-pointer' >{res.loader && res.id === n._id ? 'Loading...' : n.status}</span>
                                        }
                                        {
                                            n.status === 'deactive' && <span onClick={() => update_status('active', n._id)} className='px-2 py-0.5 bg-red-100 text-red-800 rounded-lg text-xs cursor-pointer' >{res.loader && res.id === n._id ? 'Loading...' : n.status}</span>
                                        }
                                    </td> : <td className='px-6 py-4'>
                                        {
                                            n.status === 'pending' && <span className='px-2 py-0.5 bg-blue-100 text-blue-800 rounded-lg text-xs cursor-pointer' >{n.status}</span>
                                        }
                                        {
                                            n.status === 'active' && <span className='px-2 py-0.5 bg-green-100 text-green-800 rounded-lg text-xs cursor-pointer' >{n.status}</span>
                                        }
                                        {
                                            n.status === 'deactive' && <span className='px-2 py-0.5 bg-red-100 text-red-800 rounded-lg text-xs cursor-pointer' >{n.status}</span>
                                        }
                                    </td>
                                }
                                <td className='px-6 py-4'>
                                    <div className='flex justify-start items-center gap-x-4 text-white'>
                                        <Link className='p-1.5 bg-green-500 rounded hover:shadow-lg hover:shadow-green-500/50'><FaEye /></Link>
                                        {
                                            store?.userInfo?.role === 'writer' && <>
                                                <Link to={`/dashboard/news/edit/${n._id}`} className='p-1.5 bg-yellow-500 rounded hover:shadow-lg hover:shadow-yellow-500/50'><FaEdit /></Link>
                                                <div className='p-1.5 bg-red-500 rounded hover:shadow-lg hover:shadow-red-500/50'><FaTrash /></div>
                                            </>
                                        }
                                    </div>
                                </td>
